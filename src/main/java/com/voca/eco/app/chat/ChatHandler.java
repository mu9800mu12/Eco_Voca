...
@Component // 스프링이 시작될떄, 메모리에 자동 올리는 어노테이션
...
public class ChatHandler extends TextWebSocketHandler {
    ...
    private final IMongoSerivce mongoSerivce;

    @Override
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        log.info(this.getClass().getName() + "[웹소켓 핸들러] : .handleTextMessage 시작!");
        ...
        log.info("json : " + json);
        log.info("송찬우 화이팅!");
        log.info("송찬우는 코딩고수! like 송마");
        log.info("찬우야 힘내자");

        cDTO.setRoomHash(roomNameHash);
        cDTO.setRoomName(roomName);
        mongoSerivce.insertChat(cDTO);
        ...
        log.info(this.getClass().getName() + "[웹소켓 핸들러] : .handleTextMessage 끝!");
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        clients.forEach(s -> {
            ...
            log.info("핸들러 세션 : " + s.getAttributes().get("roomNameHash"));
